 ...
 *
 * 【文案为什么硬编码英文】messages/*.json 由上游 project_sync.py 以
 * filter_messages 模式全量托管，本地新增的 key 每次同步都会被抹掉，所以桌面
 * 专属 UI 走不了 i18n。托盘菜单同理。
 *
...
export default function ExportFolderButton({ iconStyle }: { iconStyle?: React.CSSProperties }) {
  ...
  const handleClick = useCallback(async () => {
    try {
      ...
      setDir(picked);
      message.success(`Export folder set to ${picked}`);
    } catch (e) {
      console.error("choose_export_dir failed:", e);
      message.error("Could not set the export folder.");
    }
  }, [message]);
  ...
  return (
    <Tooltip title={dir ? `Export folder: ${dir}` : "Exports go to the system download folder. Click to choose another."}>
      <Button type="text" icon={<FolderOpenOutlined style={iconStyle} />} onClick={handleClick} aria-label="Choose export folder" />
    </Tooltip>
  );
}
