// 标点符号结尾的正则表达式
export const punctuationEndRegex = /[。？！…”"」】)）※]$|\.{3}$|-{3}$/;
// 特殊开头字符的正则
export const specialLineStartRegex = /^[【「\["“（(◆※]|^第.*[章节卷]$/;
...
export const chapterTitleRegex =
  /^(序章|序言|前言|卷首语|扉页|楔子|正文|终章|后记|附录|尾声|番外|[上中下][部册]册|第?\s{0,4}[\d〇零一二两三四五六七八九十百千万壹贰叁肆伍陆柒捌玖拾佰仟]+?\s{0,4}(?:章|节(?!课)|卷|集(?![合和])|幕(?![前后布])|回(?![合访忆顾应答音])|部(?![分赛游])|篇(?!张))).*/;
